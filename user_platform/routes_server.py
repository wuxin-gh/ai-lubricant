"""Public server/runtime metadata required by the upstream user portal.
...
response intentionally keeps upstream's ``{code, message, data}`` envelope
...
router = APIRouter(prefix="/api/v1/server", tags=["user-platform-server"])
