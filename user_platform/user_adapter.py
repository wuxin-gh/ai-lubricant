...
async def ensure_system_user() -> SystemUser:
    ...
    if created:
        logger.info("[user-platform] system user created id={}", user_id)
    return user
...
async def seed_bootstrap_admin() -> None:
    ...
    try:
        ...
        logger.info("[user-platform] bootstrap admin created email={}", email)
    except ValueError as exc:
        logger.warning("[user-platform] bootstrap admin skipped: {}", exc)
