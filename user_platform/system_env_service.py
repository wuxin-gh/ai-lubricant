...
def _entry_dict(row: NodeSystemEnvEntry) -> dict[str, Any]:
    return {
        ...
        "readers": [r for r in (row.readers or "").split(",") if r],
        "path": row.path or "",
...
async def refresh_system_env(user_id: str, node_id: str, *, provider: str = "") -> list[dict]:
    ...
    Two concurrent refreshes of the same node (double-click, or the
    post-install refresh racing a manual one) would interleave delete-all and
    insert-all and collide on the unique key — so the whole replacement runs
    under a per-node advisory lock inside one transaction: the second caller
    blocks until the first commits, then re-reads the now-fresh ``previous``
    and replaces it in turn. The node's report identity is
    (kind, provider, name); the unique key matches, so an operator with the
    same server name in two editors' configs lands as two rows, not a 500.
    """
    ...
    # Module-level import so tests can stub the transaction context (they run
    # without a live Tortoise connection).
    async with in_transaction(connection_name="user_platform") as conn:
        # hashtext→int4 advisory lock, transaction-scoped: released on commit,
        # so it never outlives the snapshot swap even on error.
        await conn.execute_query(
            "SELECT pg_advisory_xact_lock(hashtext($1))",
            [f"system-env-refresh:{node_id}"],
        )
        previous = await NodeSystemEnvEntry.filter(node_id=node_id).using_db(conn)
        archived_by_key = {
            (row.kind, row.name): row.archived_reference_id
            for row in previous
            if row.archived_reference_id
        }
        await NodeSystemEnvEntry.filter(node_id=node_id).using_db(conn).delete()
        for item in observed:
            ...
            readers = ",".join(
                sorted({str(r).strip() for r in (item.get("readers") or []) if str(r).strip()})
            )
            rows.append(
