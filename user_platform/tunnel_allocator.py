...
def _port_range(config: dict) -> tuple[int, int] | None:
    """The scheme's ``[lo, hi]`` port range, or None when the scheme has none.

    A scheme without a port range does no port-range restriction: each of its
    bindings must carry its own user-chosen ``remote_port``. A stale ``[0, 0]``
    (the old front-end's empty submission) is treated as unset.
    """
    rng = config.get("port_range")
    if not rng or not isinstance(rng, (list, tuple)) or len(rng) != 2 or not any(rng):
        return None
...
async def _used_ports(scheme_id: str) -> set[int]:
    ...
    A binding may hold its remote port either via auto-allocation
    (``allocated_value``) or as a user-chosen override (``remote_port``); both
    count as occupied so future allocations never hand the same port out twice.
    """
    rows = await TunnelBinding.filter(
        scheme_id=scheme_id
    ).only("allocated_value", "remote_port")
    out: set[int] = set()
    for row in rows:
        try:
...
async def allocate(scheme: TunnelScheme) -> dict:
    ...
    Raises AllocationError if the scheme's pool is exhausted, misconfigured,
    or has no port range (a range-less scheme delegates port choice to each
    binding's ``remote_port`` and must never reach auto-allocation).
    """
    ...
    if kind == "frpc" or kind == "npc":
        ...
        rng = _port_range(config)
        if rng is None:
            raise AllocationError(
                "scheme has no port_range; specify a remote_port on the binding"
            )
        lo, hi = rng
