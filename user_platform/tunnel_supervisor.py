...
class LocalTunnelSupervisor:
    ...
    async def stop_runtime(self, runtime_id: str) -> None:
        ...
        revision = self._revisions.pop(key, None)
        if proc is None or proc.returncode is not None:
            self._cleanup_conf(key, revision)
            return
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                await proc.wait()
        # 配置文件只在 spawn 时用一次，进程已停后留着是纯泄漏（temp 里的
        # mc-tunnel-confs 会随 runtime 重建/改配慢慢堆积），删掉。
        self._cleanup_conf(key, revision)

    def _cleanup_conf(self, runtime_id: str, revision: int | None) -> None:
        """删除 ``runtime-<id>-r<revision>`` 的 .toml/.conf；没写过的（cloudflared）无操作。"""
        ...
        if revision is not None:
            for suffix in (".toml", ".conf"):
                with _ctxlib.suppress(OSError):
                    (self._conf_dir / f"runtime-{runtime_id}-r{revision}{suffix}").unlink(missing_ok=True)
        # 历史堆积（含旧 revision）：runtime 已停，它名下所有配置都是孤儿。
        for path in self._conf_dir.glob(f"runtime-{runtime_id}-r*"):
            with _ctxlib.suppress(OSError):
