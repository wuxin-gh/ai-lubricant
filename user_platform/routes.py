...
These endpoints mirror the upstream user auth contract so the ported
...
router = APIRouter(prefix="/api/v1/users", tags=["user-platform"])
...
async def status(request: Request) -> dict:
    ...
    The generated upstream client expects the Go ``web.Resp`` envelope
...
# ---------------------------------------------------------------------------
# Runtime keys (our replacement for upstream's llmproxy ModelApiKey).
#
