...
def _frpc_group_toml(bindings: list[TunnelBinding], scheme: TunnelScheme) -> str:
    ...
    for binding in bindings:
        name = (binding.proxy_name or "").strip() or f"tunnel-{binding.id}"
        lines.extend([
            "",
            "[[proxies]]",
            f'name = "{name}"',
            ...
            f'remotePort = {int(binding.remote_port or binding.allocated_value or 0)}',
        ])
    return "\n".join(lines) + "\n"
...
def _npc_conf(binding: TunnelBinding, scheme: TunnelScheme) -> str:
    ...
    lines.append(f'remote_port={int(binding.remote_port or binding.allocated_value or 0)}')
    return "\n".join(lines) + "\n"
