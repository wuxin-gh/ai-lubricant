...
def _compat_conn():
    ...
    return Tortoise.get_connection("user_platform")
...
async def invalidate_binding_runtime(
    scheme: TunnelScheme, node_id: str, binding_id: str
) -> None:
    """Fence a runtime after a binding intent/config mutation.

    This is needed for grouped clients (especially frpc): a matching node
    inventory otherwise gets adopted with the old config and the removed or
    stopped proxy remains inside the still-running process.
    """
    key = runtime_key(scheme, node_id, binding_id)
    runtime = await TunnelRuntime.get_or_none(runtime_key=key)
    if runtime is not None:
        runtime.config_revision += 1
        await runtime.save(update_fields=["config_revision", "updated_at"])
...
async def reconcile_runtime(
    ...
    """Reconcile one runtime from current DB state.

    ``scheme`` is retained for call compatibility but deliberately never trusted:
    a caller may have loaded it before an administrator disabled the scheme.
    """
    del scheme
    ...
    async with lock:
        if not await _acquire_lease(runtime):
            ...
            return runtime
        try:
            return await _reconcile_runtime_locked(runtime)
        finally:
...
async def _reconcile_runtime_locked(runtime: TunnelRuntime) -> TunnelRuntime:
    await runtime.refresh_from_db()
    # Always re-read the scheme from the DB instead of trusting a caller's
    # copy: it may have been fetched before an enable-toggle landed, and
    # reconciling with a stale enabled=True copy would restart a client the
    # admin just disabled (the "switch flips back on" race). One extra query
    # per reconcile is negligible; reconciles are low-frequency.
    ...
    if scheme is None or not scheme.enabled:
        ...
        was_live = runtime.runtime_status in ("pending", "starting", "running")
        await _stop_runtime(runtime)
        if was_live:
            # Fence the killed client's late events (exited/stderr) so they
            # cannot overwrite the disabled-scheme error below.
            runtime.config_revision += 1
        runtime.runtime_status = "failed"
        runtime.error = "scheme missing or disabled"
        await runtime.save(update_fields=[
            "config_revision", "runtime_status", "error", "updated_at"
        ])
...
async def reconcile_all() -> None:
    ...
    run is recreated. Adoption additionally requires the scheme to still exist
    and be enabled — a disabled/deleted scheme's client is reconciled (stopped)
    instead, so a disable landing mid-reconcile self-heals on the next scan.
    Unknown tunnel-runtime runs are stopped as orphans.
    ...
    for target_id, runtimes in by_target.items():
        if target_id == "__main__":
            for runtime in runtimes:
                ...
                if (
                    scheme is not None and scheme.enabled
                    and _local_runtime_live(runtime, scheme)
                ):
                    # Process still alive at the current revision under an
                    # enabled scheme; do not restart on every fallback scan.
                    # A disabled/deleted scheme (or a stale process) must fall
                    # through and reconcile so the client gets stopped.
                    continue
                asyncio.create_task(reconcile_runtime(runtime))
            continue
        try:
            from .node_client import get_local_node_client
        ...
        for runtime in runtimes:
            ...
            if (
                item
                and int(item.get("revision") or 0) == runtime.config_revision
                # A disabled scheme must never be adopted as healthy even when
                # the process+revision still match: the stop pass below only
                # sees rows whose desired_state="running", and disabling a
                # scheme leaves that untouched — without this guard the client
                # stays live until the next NOTIFY happens to reconcile it.
