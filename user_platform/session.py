...
class SessionStore:
    ...
    async def delete(self, name: str, uid: str, cookie: str) -> None:
        ...
        try:
            ...
            logger.debug("[user-platform] session hdel failed", exc_info=True)
    ...
    async def delete_user_sessions(self, name: str, uid: str) -> int:
        ...
        try:
            ...
            logger.debug("[user-platform] user session hgetall failed", exc_info=True)
            return 0
        if not cookies:
            return 0
        count = 0
        for cookie in list(cookies.keys()):
            ...
            try:
                ...
                logger.debug("[user-platform] session lookup delete failed", exc_info=True)
            count += 1
        try:
            await redis.delete(redis.get_key(hash_key))
        except Exception:
            logger.debug("[user-platform] session hash delete failed", exc_info=True)
        return count
