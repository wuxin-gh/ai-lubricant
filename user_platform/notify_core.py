...
async def _match_channels(
    ...
    For user-side default events, provision the in-app notify-center binding
    before matching. This makes the simple mobile mode work for existing users
    even if they never opened the notification settings page: the first task
    terminal/pause or node-online event creates the defaults, then the same
    occurrence is matched and written to ``notifications``. Provisioning is
    idempotent and never re-subscribes an event the user previously disabled.
    """
    if owner_type == "user" and event_type in ("task.ended", "task.paused", "node.online"):
        try:
            from .notify_service import notify_service

            await notify_service.ensure_default_app_notifications(str(owner_id or ""))
        except Exception:  # noqa: BLE001 — notification setup cannot break outbox drain
            logger.exception("[notify] ensure user app defaults failed for {}", owner_id)
    ...
    async with PostgresClient.pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            "c.id, c.kind, c.webhook_url, c.secret, c.headers, c.target_id "
...
async def _deliver_mobile_push(
    ...
    """Push one event to the device bound to a ``mobile_push`` channel.

    Returns ``(send_status, error)`` — ``skipped`` when the device is
    missing/disabled, ``success``/``failed`` from the Expo response. A
    ``DeviceNotRegistered``-class response retires the device here so the next
    event stop-matches it.
    """
    from .notify_push import (
        build_push_payload,
        disable_device,
        load_device_by_channel,
        send_expo_push,
    )

    device = await load_device_by_channel(channel_target_id)
    if device is None:
        return "skipped", "device missing or disabled"
    token = device.get("push_token") or ""
    if not token:
        return "skipped", "no push token"
    severity = str(envelope.get("severity") or params.get("severity") or "info")
    title = envelope.get("title") or _EVENT_TITLES.get(event_type, event_type)
    message = envelope.get("message") or ""
    # Carry only the deep-link fields the app reads on tap — not the full
    # envelope (it may contain request-log ids and internals the device never
    # renders). Route is what the app uses to navigate on tap.
    deep_link = {k: v for k, v in params.items() if k in (
        "task_id", "task", "status", "result", "node_id", "provider",
    )}
    deep_link["event_type"] = event_type
    # Tap takes the user to the task detail if this is a task event, else the
    # notification center.
    route = "/(tabs)/tasks"
    if event_type.startswith("task") and params.get("task_id"):
        route = f"/task/{params['task_id']}"
    elif event_type.startswith("node"):
        route = "/nodes"
    deep_link["route"] = route
    payload = build_push_payload(
        token, title=str(title), body=str(message), data=deep_link, severity=severity,
    )
    result = await send_expo_push(payload)
    if result.device_gone:
        await disable_device(device.get("id"), result.error or "device gone")
    return ("success" if result.ok else "failed", result.error)
...
async def _consume_one(outbox_id: Any) -> None:
    ...
    for ch in channels:
        ...
        if kind == NOTIFY_CENTER_KIND:
            ...
            continue
        if kind == MOBILE_PUSH_KIND:
            # Per-device push: transport lives in notify_push. A missing/disabled
            # device is a skipped delivery, not a failure — send-log records it.
            try:
                push_status, push_error = await _deliver_mobile_push(
                    ch.get("target_id"), row["event_type"], envelope, params
                )
                logs.append((channel_id, push_status, push_error))
            except Exception as exc:  # noqa: BLE001
                status = "failed"
                error = str(exc) or exc.__class__.__name__
                logs.append((channel_id, "failed", error))
                logger.info("[notify] outbox {} mobile push failed: {}", outbox_id, error)
            continue
        if not webhook_url:
