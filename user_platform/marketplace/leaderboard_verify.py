...
from ..git_clients import github_rate_limit_message
    ...
    if isinstance(skill, dict) and isinstance(skill.get("entries"), list):
        ...
        for entry in skill["entries"]:
            ...
            if not full_name or not full_path:
                ...
                continue
            # raw 直链优先（不占 API 配额）；raw 失败再用 Contents API 确认，兼容私有仓
            # 或 raw CDN 异常。ref 为 HEAD/空等无法由 raw 稳定解析的情况直接走 API。
            if ref in ("", "HEAD"):
                ok, err = await _head_ok(
                    f"https://api.github.com/repos/{full_name}/contents/{full_path}?ref={ref}"
                )
            else:
                ok, err = await _head_ok(
                    leaderboard_probe.raw_content_url(full_name, ref, full_path)
                )
                if not ok:
                    ok, err = await _head_ok(
                        f"https://api.github.com/repos/{full_name}/contents/{full_path}?ref={ref}"
