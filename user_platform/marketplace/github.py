...
from ..git_clients import GitClientError, _HTTP_TIMEOUT, github_rate_limit_message
...
class MarketplaceGitHub:
    ...
    async def read_json(self, path: str, *, use_cache: bool = True) -> tuple[Any, str]:
        ...
        if resp.status >= 400:
            ...
            _raise_github_error(resp, f"GET {path} returned HTTP {resp.status}", text)
    ...
    async def write_json(self, path: str, data: Any, message: str) -> None:
        ...
        if resp.status >= 400:
            text = await resp.text()
            _raise_github_error(resp, f"PUT {path} returned HTTP {resp.status}: {text[:300]}", text)
    ...
    async def delete_file(self, path: str, message: str) -> bool:
        ...
        if resp.status >= 400 and resp.status != 404:
            text = await resp.text()
            _raise_github_error(resp, f"DELETE {path} returned HTTP {resp.status}: {text[:300]}", text)
        if resp.status < 400:
            _cache_invalidate(_cache_key(self._settings, path))
...
def _raise_github_error(resp: Any, fallback: str, body: str = "") -> None:
    """限流→直白原因 + 恢复时间；其余按调用方给的文案抛。

    限流消息以 ``RATE_LIMIT_PREFIX`` 开头，调用方无需自行拼接。
    """
    rate_limited = github_rate_limit_message(resp, body)
    if rate_limited:
        raise GitClientError(rate_limited)
    raise GitClientError(fallback)
    ...
    async with _http_session() as session:
        async with session.post(
            ...
            if resp.status not in (200, 201):
                text = await resp.text()
                _raise_github_error(resp, f"create_release failed: HTTP {resp.status} {text[:300]}", text)
...
async def upload_release_asset(
    ...
    """上传 asset 到 Release。同名 asset 已存在时先删后传，保证重传幂等。

    upload_url 格式：``https://uploads.github.com/repos/{owner}/{repo}/releases/{id}/assets{?name,label}``
    需要移除模板参数部分，添加 ``?name=<filename>``。

    同版本重传（部分文件已上到 Release 后失败，服务端重启/过期后前端整包重传、或
    管理员关掉弹框后手动重传）会撞 GitHub 的同名 asset 422——新 job 的文件都是
    pending，不会走「已 done 跳过」路径。这里对 422 删掉旧 asset 重传一次，重传
    后直链指向新 asset，不会留孤儿。
    """
    from ..git_clients import _http_session
    ...
    async def _post() -> tuple[int, Any, Any]:
        async with _http_session() as session:
            async with session.post(
                url,
                data=content,
                ...
                return resp.status, await resp.json(), resp

    status, payload, resp = await _post()
    if status == 422:
        # 同名 asset 已存在：删旧传新（找不到 release/asset 就原样报错）。
        ids = _parse_release_ids(base_url)
        if ids is not None:
            owner, repo, release_id = ids
            if await _delete_release_asset_by_name(owner, repo, token, release_id, filename):
                status, payload, resp = await _post()
    if status not in (200, 201):
        _raise_github_error(resp, f"upload_release_asset {filename} failed: HTTP {status} {str(payload)[:300]}", str(payload))
    return payload


def _parse_release_ids(base_url: str) -> tuple[str, str, str] | None:
    """从 uploads.github.com 的 upload_url base 解析 (owner, repo, release_id)。

    upload_url base 形如 ``https://uploads.github.com/repos/{owner}/{repo}/releases/{id}/assets``。
    """
    prefix = "https://uploads.github.com/repos/"
    suffix = "/assets"
    if not (base_url.startswith(prefix) and base_url.endswith(suffix)):
        return None
    parts = base_url[len(prefix):-len(suffix)].split("/")
    # owner / repo / "releases" / release_id
    if len(parts) != 4 or parts[2] != "releases" or not parts[3].isdigit():
        return None
    return parts[0], parts[1], parts[3]


async def get_release_by_id(owner: str, repo: str, token: str, release_id: str) -> dict | None:
    """按 id 获取 Release 信息（含 assets 列表），不存在返回 None。"""
    from ..git_clients import _http_session

    url = f"https://api.github.com/repos/{quote(owner, safe='')}/{quote(repo, safe='')}/releases/{release_id}"
    headers = {
        "Authorization": f"token {token}",
        "Accept": "application/vnd.github.v3+json",
        "User-Agent": "ai-lubricant-marketplace",
    ...
    async with _http_session() as session:
        async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=30)) as resp:
            ...
            if resp.status != 200:
                text = await resp.text()
                _raise_github_error(resp, f"get_release_by_id {release_id} failed: HTTP {resp.status} {text[:300]}", text)
            return await resp.json()


async def delete_release_asset(owner: str, repo: str, token: str, asset_id: int | str) -> bool:
    """删除 Release 上的 asset，成功（204）返回 True。"""
    from ..git_clients import _http_session

    url = f"https://api.github.com/repos/{quote(owner, safe='')}/{quote(repo, safe='')}/releases/assets/{asset_id}"
    headers = {
        "Authorization": f"token {token}",
        "Accept": "application/vnd.github.v3+json",
        "User-Agent": "ai-lubricant-marketplace",
...
async def _delete_release_asset_by_name(owner: str, repo: str, token: str, release_id: str, filename: str) -> bool:
    """删掉 Release 上同名 asset（重传前置），没有同名时返回 False。"""
    release = await get_release_by_id(owner, repo, token, release_id)
    if not release:
        return False
    for asset in release.get("assets") or []:
        if asset.get("name") == filename:
            return await delete_release_asset(owner, repo, token, asset["id"])
    return False
...
async def get_release_by_tag(owner: str, repo: str, token: str, tag: str) -> dict | None:
    ...
    async with _http_session() as session:
        async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=30)) as resp:
            ...
            if resp.status != 200:
                text = await resp.text()
                _raise_github_error(resp, f"get_release_by_tag {tag} failed: HTTP {resp.status} {text[:300]}", text)
            return await resp.json()
...
async def get_latest_release(owner: str, repo: str, token: str = "") -> dict | None:
    ...
    async with _http_session() as session:
        async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=30)) as resp:
            ...
            if resp.status != 200:
                text = await resp.text()
                _raise_github_error(resp, f"get_latest_release failed: HTTP {resp.status} {text[:300]}", text)
            return await resp.json()
