    ...
    try:
        ...
        try:
            ...
            await marketplace_sync_run_store.record_run(
                "agentscope", ok=ok, detail=detail, logs=logs, run_by="manual",
        ...
        try:
            import marketplace_sync_run_store
            from .source_config import record_source_sync
            await marketplace_sync_run_store.record_run("agentscope", ok=False, detail=str(exc)[:200], logs=logs, run_by="manual")
            await record_source_sync("agentscope", ran_at)
