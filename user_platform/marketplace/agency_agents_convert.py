...
- ``sync_agency_agents`` 逐条落 ``marketplace_leaderboard_items`` 候选池
  （source=agency-agents / agency-agents-zh，**draft**，需管理端显式发布才对
  用户可见）。早期直写 prompts 模块（``routes.import_data``）的实现已废弃——
  那批 status=published 的旧行见 ``script/purge_orphaned_marketplace_items.py``。
...
# 英文源：用户指定的基线 commit；换基线走 sync_agency_agents(ref=...)。
DEFAULT_REF = "3c9588880b7cafaec325a104899fd8bbe27e7d72"
# 中文源（jnMetaCode/agency-agents-zh）与英文源历史分叉——英文源的 pinned sha 在
# fork 里不存在（404 实证），所以中文源默认跟它自己的 main 分支（社区源持续更新，
# 跟分支合理；分支 ref 走 1 小时 TTL 缓存）。要钉死某次翻译基线用 config 的
# agency_agents_zh_ref 传 sha。
ZH_REPO = "jnMetaCode/agency-agents-zh"
DEFAULT_ZH_REF = "main"
MODULE = "prompts"
DEFAULT_VERSION = "1.0.0"
DEFAULT_PROVIDERS = ["claude", "codex", "opencode", "cursor", "gemini"]
...
async def _fetch_tarball(ref: str, repo: str = SOURCE_REPO, *, proxy_id: str = "") -> bytes:
    """整仓 tarball，经 proxy_manager（codeload 主路，api.github 备路；各重试 3 次）。

    ``repo`` 默认英文源（``agency-agents``）；中文源传 ``jnMetaCode/agency-agents-zh``。
    两个源 owner 不同（英文 msitarzewski / 中文 jnMetaCode），所以这里按 ``owner/repo``
    整串拆 owner，不假设 ``SOURCE_OWNER``。``proxy_id`` 留空回落全局。
    """
    import asyncio

    from providers.proxy_manager import get_proxy_manager

    # repo 两种形态："agency-agents"（name-only，英文源默认）或
    # "jnMetaCode/agency-agents-zh"（owner/name，中文源——owner 与英文源不同）。
    # 拆成 (owner, name)：name-only 回落 SOURCE_OWNER。
    text = str(repo).strip()
    if "/" in text:
        owner, name = (part.strip() for part in text.split("/", 1))
    else:
        owner, name = SOURCE_OWNER, (text or SOURCE_REPO)
    full_repo = f"{owner}/{name}"
    urls = [
        f"https://codeload.github.com/{owner}/{name}/tar.gz/{ref}",
        f"https://api.github.com/repos/{owner}/{name}/tar/{ref}",
    ]
    manager = get_proxy_manager()
    pid = proxy_id or mp_config.settings.proxy_id or None
        ...
        for attempt in range(3):
            try:
                resp = await manager.request(
                    ...
                    timeout=_FETCH_TIMEOUT, proxy_config_id=pid,
                )
                if resp.status == 404:
    ...
    raise RuntimeError(f"拉取 {full_repo}@{ref} 失败: {last_err}")
...
def _extract_source(data: bytes) -> dict[str, str]:
    ...
    divisions.json **不强制**：agency-agents-zh 没有（fork 分叉），缺失时由
    ``_parse_divisions`` 按 agent 路径合成 division 集合——元文件缺失不再让整源
    同步 0 条。真正的硬条件是至少有一个 agent .md。
    """
    files: dict[str, str] = {}
    ...
    if not any(rel != "divisions.json" for rel in files):
        raise RuntimeError("tarball 里没有任何 agent .md 文件")
    return files
...
async def fetch_source(ref: str, flush: bool, *, repo: str = SOURCE_REPO, proxy_id: str = "") -> dict[str, str]:
    """带缓存的源读取（网络走 proxy_manager）。sha ref 永久复用；分支 ref 一小时内复用。

    ``repo`` 默认英文源；中文源传 ``jnMetaCode/agency-agents-zh``，缓存按 repo 分文件。
    ``proxy_id``：本源专用代理（留空回落全局 ``proxy_id``）。
    """
    import asyncio
    ...
    if not flush and cache.exists():
        try:
            ...
            pass  # 缓存坏了就当没有，重新拉
    data = await _fetch_tarball(ref, repo=repo, proxy_id=proxy_id)
...
def _synthesize_divisions(files: dict[str, str]) -> dict[str, dict]:
    """中文社区源没有 divisions.json 时，从实际 agent 目录确定 division 集合。

    agency-agents-zh 的 ``CATALOG.md`` 是结构化目录，但转换只需要 division key；
    tarball 已按 ``<division>/<agent>.md`` 白名单筛过，直接从路径首段收集即可。
    label 没有额外机器清单时回落目录名，保证单个坏文件不会拖垮整源。
    """
    ...
    for rel in files:
        ...
        divisions.setdefault(division, {"label": division})
    return divisions


def _parse_divisions(files: dict[str, str], repo: str) -> dict[str, dict]:
    """读取 divisions.json；中文源缺失时按 agent 路径合成，不能让一个元文件失败拖全量。"""
    ...
    if raw:
        try:
            ...
            pass
    return _synthesize_divisions(files)
...
def convert(
    ...
    *,
    ...
    divisions 优先读 divisions.json（英文源）；缺失/损坏时按 agent 路径合成
    （agency-agents-zh 没有 divisions.json——fork 分叉删了，之前直接 raise 让
    整源 0 条入库）。
    """
    divisions = _parse_divisions(files, repo)
...
async def sync_agency_agents(
    *, ref: str = "", flush: bool = False, dry_run: bool = False,
    ...
    ``repo`` 默认英文源（agency-agents）；中文源传 ``jnMetaCode/agency-agents-zh``。
    ``ref`` 留空按源取默认：英文源=基线 commit（DEFAULT_REF）；中文源=其 main 分支
    ...
    is_zh = "agency-agents-zh" in str(repo)
    source_name = "agency-agents-zh" if is_zh else "agency-agents"
    result_store = _last_result_zh if is_zh else _last_result
    ref = str(ref or "").strip() or (DEFAULT_ZH_REF if is_zh else DEFAULT_REF)
    logs: list[dict[str, str]] = []

    def _log(phase: str, detail: str) -> None:
        logs.append({"ts": datetime.now(timezone.utc).isoformat(), "phase": phase, "detail": detail})

    # 本源专用代理（留空回落全局 proxy_id）。
    from .source_config import get_source_config_async, effective_proxy_id, record_source_sync
    source_cfg = await get_source_config_async()
    own_proxy_key = "agency_agents_zh_proxy_id" if is_zh else "agency_agents_proxy_id"
    proxy_id = effective_proxy_id(source_cfg, own_proxy_key)
    ...
    try:
        ...
        files = await fetch_source(ref, flush, repo=repo, proxy_id=proxy_id)
        ...
        result_store.update({"ran_at": started, "ok": ok, "detail": detail, "logs": logs})
        # 执行记录落 DB（重启后面板仍能看到最新一份日志）+ 上次同步时间随配置持久化。
        try:
            import marketplace_sync_run_store
            await marketplace_sync_run_store.record_run(
                source_name, ok=ok, detail=detail, logs=logs, run_by="manual",
                counts={"converted": report["converted"], "written": written, "failed": len(import_failed)},
            )
            await record_source_sync(source_name, started)
        ...
        try:
            import marketplace_sync_run_store
            await marketplace_sync_run_store.record_run(
                source_name, ok=False, detail=f"同步失败：{exc}"[:300], logs=logs, run_by="manual",
            )
            await record_source_sync(source_name, started)
...
async def sync_loop(repo: str = SOURCE_REPO) -> None:
    """后台定时同步。开关关闭时空转（每小时复查配置，便于热开启）。

    与 leaderboard_sync.sync_loop 同款 asyncio 后台循环（不是进程/cron）：每
    ``interval_hours`` 跑一次 ``sync_agency_agents(repo=...)``。同源已在跑时由
    ``_progress.running`` 挡掉（sync_agency_agents 内部判重），不并发跑。
    """
    import asyncio
    from .source_config import get_source_config_async
    from .source_config import _SOURCE_KEYS  # noqa: F401 — 仅复用 source 名映射

    source_name = "agency-agents-zh" if repo != SOURCE_REPO else "agency-agents"
    while True:
        try:
            cfg = await get_source_config_async()
            enabled = bool(cfg.get("agency_agents_zh_enabled")) if repo != SOURCE_REPO \
                else bool(cfg.get("agency_agents_enabled"))
            if enabled:
                hours = int(cfg.get("agency_agents_zh_interval_hours")
                            if repo != SOURCE_REPO else cfg.get("agency_agents_interval_hours") or 24)
                if not _progress.get("running"):
                    await sync_agency_agents(repo=repo)
                await asyncio.sleep(max(1, hours) * 3600)
                continue
        except asyncio.CancelledError:
            raise
        except Exception as exc:  # noqa: BLE001 — 循环不能因一次失败退出
            logger.warning("[{}] sync_loop error: {}", source_name, exc)
