...
_TTL_SECONDS = 30 * 60
# 惰性 sweep 节流：get_job 被前端 2s 轮询高频调用，sweep 每 60s 至多跑一次。
...
def _maybe_sweep() -> None:
    """get_job 惰性触发 sweep（60s 节流）。本模块无独立后台任务——前端轮询天然
    高频打 get_job，顺路清理即可；无上传活动时残留几条终态 job 无害。"""
...
_UPLOAD_TMP_BASE: str | None = None


def _tmp_base() -> str:
    """上传暂存根目录：默认系统 temp（Windows 即 C:\\Users\\<u>\\AppData\\Local\\Temp），
    可用 ``AI_LUBRICANT_UPLOAD_TMPDIR`` 指向空间更足的盘——节点发行文件单文件上限 2GB，
    C 盘 temp 常装不下。首次调用后缓存，进程内不再变。"""
    global _UPLOAD_TMP_BASE
    if _UPLOAD_TMP_BASE is None:
        _UPLOAD_TMP_BASE = (os.environ.get("AI_LUBRICANT_UPLOAD_TMPDIR") or "").strip() or tempfile.gettempdir()
    return _UPLOAD_TMP_BASE


def tmp_dir(job_id: str) -> str:
    """每个 job 一个临时目录，存放第①阶段收到的文件。"""
    return os.path.join(_tmp_base(), f"ai-lubricant-upload-{job_id}")
...
def sweep_orphan_dirs() -> None:
    """回收暂存根目录里残留的 ``ai-lubricant-upload-*`` 孤儿目录。

    内存态 ``_jobs`` 进程重启即空：写盘半截崩溃 / kill / 断流未清的暂存目录，在
    ``_jobs`` 里查不到对应 job_id 就是孤儿。_maybe_sweep（每 60s）+ lifespan 启动
    各调一次，避免半截文件把磁盘慢慢吃满。活着的 job（仍在 _jobs 里）跳过。
    """
    base = _tmp_base()
    try:
    ...
    import shutil
    for name in entries:
        if not name.startswith("ai-lubricant-upload-"):
            continue
        jid = name[len("ai-lubricant-upload-"):]
        if jid in _jobs:
            continue
        try:
...
def sweep_expired() -> None:
    """清掉超时的 job 及其临时目录（get_job 惰性触发，见 _maybe_sweep）。

    仍活着的转传任务不杀：2GB APK / URL 模式下载超 30 分钟是合法长任务，杀了会让
    前端永远等不到终态。只回收终态（done/failed）或任务已死/从未起过的过期 job。
    """
    now = _now()
    expired = []
    for jid, j in _jobs.items():
        if now - j["created_at"] <= _TTL_SECONDS:
            continue
        t = _tasks.get(jid)
