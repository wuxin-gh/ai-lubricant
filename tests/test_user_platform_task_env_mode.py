...
def _task_with(env_mode: str | None, env_id: str | None = None, **extra) -> Task:
    cfg: dict = {}
    if env_mode:
        cfg["env_mode"] = env_mode
    if env_id:
...
def test_build_session_shared_active_skill_subset():
    s = TaskService()._build_session(
        {"cli_name": "claude"},
        _task_with("shared", "prod", active_skills=["one", "two"], active_plugins=["plug"]),
    )
    assert s["activeSkills"] == ["one", "two"]
    assert s["activePlugins"] == ["plug"]


def test_build_session_system_active_skill_subset():
    s = TaskService()._build_session(
        {"cli_name": "claude"},
        _task_with("system", active_skills=["operator-skill"]),
    )
    assert s["activeSkills"] == ["operator-skill"]


def test_build_session_active_empty_means_environment_default():
    s = TaskService()._build_session(
        {"cli_name": "claude"},
        _task_with("shared", "prod", active_skills=[]),
    )
    assert "activeSkills" not in s
