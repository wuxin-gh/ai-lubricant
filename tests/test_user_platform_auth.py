"""Phase A auth-domain tests for the upstream compatibility layer.
...
* session key construction (hash key + lookup key) matching upstream.
"""
...
def test_bcrypt_round_trip():
    from user_platform import crypto
...
def test_mount_routes_noop_when_disabled(monkeypatch, tmp_path):
    ...
    import user_platform as compat
...
def test_session_key_construction():
    from user_platform import session
...
def test_validate_new_credentials_rejects_invalid_email():
    ...
    import pytest
    from user_platform.validation import validate_new_credentials
