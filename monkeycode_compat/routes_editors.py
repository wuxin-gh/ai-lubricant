...
router = APIRouter(prefix="/api/v1/users/editors", tags=["monkeycode-editors"])
admin_router = APIRouter(prefix="/api/v1/teams/editors", tags=["monkeycode-editor-admin"])
...
def _issue_workflow_mcp_entry(token: str) -> dict:
    ...
    if not base:
        raise RuntimeError("未配置 issue-workflow MCP 服务地址")
    return {
        "name": "issue-workflow",
        "type": "sse",
...
async def update_editor(
    ...
    try:
        from .resource_reference_service import resolve_prompt_for_user, resolve_reference_specs
        ...
        if "mcp_config" in patch:
            patch["mcp_config"] = await resolve_reference_specs(
                str(user.id), team_id, "mcp", patch["mcp_config"], request_base_url=base_url,
            )
            ...
            try:
                if "mcp_config" in patch:
                    await client.apply_node_session_mcps(
                        node_session_id,
                        _merge_mcp_config(updated.get("mcp_config") or [], sess.get("mcp_overlay_json") or []),
                    )
                if "skill_config" in patch:
...
async def _create_editor_session_core(
    ...
    try:
        ...
        mcps = _merge_mcp_config(editor.get("mcp_config") or [], mcp_overlay)
