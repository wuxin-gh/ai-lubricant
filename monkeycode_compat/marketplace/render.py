...
def render_node_release(manifests: list[dict]) -> tuple[dict, list[str]]:
    """node-releases/version.json：返回 ``(payload, errors)``。

    草稿与测试版本排除——它们能在市场管理里看，但不该让线上节点自动认为需要升级。
    无可用版本仍渲染空 schema 文件，让消费侧区分「暂无版本」与「仓库不是市场」。
    """
    payload: dict = {
    ...
    for manifest in manifests:
        if not isinstance(manifest, dict):
            continue
        if manifest.get("status") != "published" or manifest.get("test_version"):
            continue
