...
def _emit_publish_failed(module: str, error: str) -> None:
    ...
    try:
        ...
        emit_notification_background(
            ...
            message=f"市场发布失败（{module}）：{(error or '')[:200]}",
        )
...
async def _publish_module_locked(store, client: MarketplaceGitHub, module: str, jobs: list[dict]) -> None:
    ...
    for item_id, job in by_item.items():
        ...
        if item_id in current_ids:
            ...
            if row and isinstance(row.get("manifest"), dict):
                ...
                # draft/published 都镜像——消费侧 version.json 投影才负责排除 draft。
    ...
    # ② index：从 store 当前 summaries 渲染。
    summaries = await store.list_summaries(module)
    await client.write_json(
    ...
    # ④ node/mobile 版本投影：草稿/测试版本不进 version.json（升级链路只认已发布）。
