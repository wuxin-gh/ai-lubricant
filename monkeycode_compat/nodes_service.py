...
def _default_node_bin_candidates() -> list[Path]:
    ...
    """
    # monkeycode_compat/ → ai-lubricant/
...
def render_install_script(
    ...
    *,
    ...
    rt_lines = "\n".join([
        f'RT_URL={_sh_squote(str(rt_entry.get("url") or ""))}',
        f'RT_SHA={_sh_squote(str(rt_entry.get("sha256") or ""))}',
    ])
...
# tar.gz whose top-level directory is exactly ``runtime/``; extract it into Go's
# stateDir ($XDG_CONFIG_HOME/agent-compose/node or ~/.config/agent-compose/node)
# so RuntimeDir() sees runtime/dist/cli.js.
if [ -z "$rt_url" ]; then
  echo "error: 当前没有已发布的 agent-compose runtime（node-runtime.tar.gz），请在市场管理页上传该通用包" >&2
  exit 1
fi
state_dir="${{AGENT_COMPOSE_NODE_STATE_DIR:-${{XDG_CONFIG_HOME:-$HOME/.config}}/agent-compose/node}}"
mkdir -p "$state_dir"
...
if [ -n "$rt_sha" ]; then
  actual="$(sha256sum "$rt_archive" | awk '{{print $1}}')"
...
UNIT
  sed -i "s#__ROOT__#$root#g" "$HOME/.config/systemd/user/agent-compose-node.service"
  ...
  if command -v crontab >/dev/null 2>&1; then
    ( crontab -l 2>/dev/null | sed '/agent-compose-node/d'; echo "@reboot $root/start-node.sh >> $root/node.log 2>&1" ) | crontab -
  ...
  nohup "$root/start-node.sh" >> "$root/node.log" 2>&1 &
