...
- 项目形态：web 前后端 / 移动 app / 桌面 / 库 / CLI / 容器化
...
async def detect_stack(
    ...
    *,
    ...
    # 移动端框架（Expo 托管工作流常无 AndroidManifest.xml，靠框架兜底）。
    if framework_set & _MOBILE_FRAMEWORKS:
        project_types.append("mobile_app")
    # 原生工程证据：iOS 的 Podfile/.xcodeproj、Android 的 AndroidManifest.xml。
    # 注意 pubspec.yaml 不再单独触发——纯 dart 包也有 pubspec，只有命中
    # flutter 框架（sdk: flutter）才是移动 app。
    if ("podfile" in presence) or ("androidmanifest.xml" in presence) or any(
        # .xcodeproj 是目录：调用方常只传文件路径，按路径段识别。
