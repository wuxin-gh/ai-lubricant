...
Mirrors the MonkeyCode task handler contract so the ported frontend behaves
...
router = APIRouter(prefix="/api/v1/users/tasks", tags=["monkeycode-tasks"])
...
async def _resolve_task_resource_configs(
    ...
    for ids_key, public_key, resource_type in (
        ...
        bindings: list[dict] = [
            {"resource_id": str(rid)} for rid in (extra.get(ids_key) or []) if rid
        ]
    ...
    if isinstance(mcp_explicit, list) and mcp_explicit:
        try:
            patch["mcp_config"] = _dedup_by_name(await resolve_reference_specs(
                str(user.id), team_id, "mcp", mcp_explicit, request_base_url=base_url,
            ))
        except ValueError as exc:
            raise HTTPException(status_code=403, detail=f"资源未授权或配置无效: {exc}") from exc
    return patch
...
async def switch_task_model(
    ...
    try:
        ...
        detail = {
            ...
            "model_not_available_for_key": "目标模型不在任务可用范围内，请先通过「添加模型」加入",
