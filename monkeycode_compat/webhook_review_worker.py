...
async def _review_mcp_entry(event: ProjectWebhookEvent) -> dict | None:
    try:
        ...
        from monkeycode_compat import config as compat_config
...
async def _resolve_legacy_review_config(hook: ProjectWebhook, project: Project) -> None:
    ...
    for editor in editors:
        ...
        mcps = _merge_named(mcps, list(editor.get("mcp_config") or []))
...
async def execute_claude_delegate(
    ...
    req: dict = {
        ...
        "_session_mcps": _merge_named(hook.review_mcp_config, mcp_overlay),
