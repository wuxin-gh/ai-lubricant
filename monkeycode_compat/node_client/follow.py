"""Data-side Connect server-stream passthrough for FollowNodeSession.

Preserves the Connect streaming framing (5-byte prefix + protobuf) by relaying
the HTTP body verbatim; the control process owns sinks and the live Registry.
"""
...
_PATH = f"/{_SERVICE}/FollowNodeSession"
...
async def proxy_follow_asgi(scope: dict, receive, send, *, base_url: str, token: str) -> None:
    ...
    try:
        ...
        logger.debug("[node-client] follow proxy ended")
