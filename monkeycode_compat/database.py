...
MODEL_MODULES = [
    ...
    "monkeycode_compat.models_environment",
...
async def init_database() -> None:
    ...
    config["connections"] = {"monkeycode_compat": settings.database_url}
    ...
    logger.debug("[monkeycode-compat] tortoise schemas ready")
...
async def _drop_retired_agent_skill_storage() -> None:
    ...
    conn = Tortoise.get_connection("monkeycode_compat")
    ...
    logger.info("[monkeycode-compat] dropped retired table mc_agent_skill_bindings")
    ...
    * Safe on populated tables — every added column is made NULLable regardless
    ...
    for model in app_models.values():
        ...
        for field_name, column in meta.fields_db_projection.items():
            ...
            logger.info(
                "[monkeycode-compat] added missing column {}.{} ({})",
                table,
                column,
                sql_type,
            )
...
async def _migrate_project_issue_workflow() -> None:
    ...
    logger.debug("[monkeycode-compat] project issue workflow migration ready")
...
async def _migrate_task_bootstrap() -> None:
    ...
    logger.debug("[monkeycode-compat] task bootstrap migration ready")
...
async def _migrate_tunnel_runtime() -> None:
    ...
    logger.debug("[monkeycode-compat] tunnel runtime migration ready")
