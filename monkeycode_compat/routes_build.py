...
#
# Appium WebDriverAgent 固定源 + 固定 tag。⚠️ v2.5.5 是「待真机验证」占位——
# 尚未在真实 mac 跑过，首次真机构建请按清单核对并回填：
#   1. tag 可拉取（git ls-remote --tags 看 v2.5.5 是否存在/是否目标版）
#   2. scheme 名是否为 WebDriverAgentRunner（xcodebuild -list 核对）
#   3. 产物路径是否 build/Build/Products/*/*-iphonesimulator/WebDriverAgentRunner-Runner.app
#      （Xcode 版本差异可能落在 DerivedData；必要时加 -derivedDataPath build 收敛）
#   4. zip 后 unsigned.ipa 是否落在 srcDir（artifact_glob 按 here 找）
# 有出入时改这里的 STEPS / GLOB 即可，不动架构。签名一律走 go-ios ASC 路径，
# 所以这里 CODE_SIGN_* 全关。
#
# build-for-testing 出 .xctestrun + .app；打 .ipa 只是 Payload 目录 zip。
# glob 写宽一点兜底不同 Xcode 版本的 Debug/Release 子目录。
WDA_RECIPE = {
    "source_url": "https://github.com/appium/WebDriverAgent.git",
    "source_ref": "v2.5.5",  # 待真机验证；真机跑通后按实际可用 tag 固化
    ...
    "steps": [
        ...
        "mkdir -p Payload && cp -r build/Build/Products/*/*-iphonesimulator/WebDriverAgentRunner-Runner.app Payload/",
        "zip -qry unsigned.ipa Payload",
    ],
    # zip 在 srcDir 产出 unsigned.ipa；写宽一点兜底产物路径差异。
